...
export interface InitMarketArgs {
  ...
  /** Per-market admin limit: max maintenance fee per slot (u128). Omit = use max (uncapped). */
  maxMaintenanceFeePerSlot?: bigint | string;
  /** Per-market admin limit: max risk reduction threshold (u128). Omit = use max (uncapped). */
  maxRiskThreshold?: bigint | string;
  /** Per-market admin limit: min oracle price cap e2bps (u64). Omit = 0. */
  minOraclePriceCapE2bps?: bigint | string | number;
...
export function encodeInitMarket(args: InitMarketArgs): Buffer {
  ...
  //         conf_filter_bps(2) + invert(1) + unit_scale(4) + initial_mark_price_e6(8) +
  //         max_maintenance_fee_per_slot(16) + max_risk_threshold(16) + min_oracle_price_cap_e2bps(8) + RiskParams(...)
  const maxMaintenanceFeePerSlot = args.maxMaintenanceFeePerSlot ?? U128_MAX;
  const maxRiskThreshold = args.maxRiskThreshold ?? U128_MAX;
  const minOraclePriceCapE2bps =
    args.minOraclePriceCapE2bps != null
      ? typeof args.minOraclePriceCapE2bps === "number"
        ? BigInt(args.minOraclePriceCapE2bps)
        : args.minOraclePriceCapE2bps
      : 0n;
  return Buffer.concat([
    ...
    encU128(maxMaintenanceFeePerSlot),
    encU128(maxRiskThreshold),
    encU64(minOraclePriceCapE2bps),
